...
function bubble(a) {
    ...
    if(a.href.search("stv-nas") != -1) {
        ...
        if ( (test.search("START") == -1)  && (test.search("STOP") == -1)  && (test.search("RESET") == -1) ) {
            ...
            req.open('GET', 'http://ukrav/results/get_result_rav_bubble.php?t='+table+'&trun='+test_run_name+'&tnum='+test, false);
            req.send(null);  

            if(req.status == 200) {
                pop_html =  pop_html +req.responseText;
            }
            else {
                pop_html =  pop_html +' Error in UKRAV result server, XMLHttpRequest return : '+req.status;
            }
        }
        else {
            pop_html = pop_html + "<b>Test:</b> "+ test +"<br />";                    
